...
import {getDataListPaging} from "../../springApi/dataSpringBootApi";

// MemberListPaging.js 처리 정의
const DataListPaging = () => {
    ...
    useEffect(() => {
        ...
        getDataListPaging(currentPage, size)
            .then((res) => {
                // 백엔드로부터 받은 결과값(res.data)을 상태변수에 저장하기
                setDataListData(res.data.content);
            ...
            <div>
                ...
                <table border="1" style={{width : "100%"}}>
                    ...
                    <tbody>
                        ...
                        */}
                        {dataListData.map((data) => (
                            <tr>
                                <td>
                                    {/* React에서 제공하는 Link 태그 사용 */}
                                    <Link to={`/data/view_paging/${data.data_id}/${currentPage}`}>
                                        {data.data_id}
                                    </Link>
                                </td>
                                <td>{data.mem_id}</td>
