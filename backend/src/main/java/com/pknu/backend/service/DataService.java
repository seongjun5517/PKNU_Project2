...
import com.pknu.backend.repository.DataRepository;
...
public class DataService {
    ...
    public List<Data> getDatalist() {
        ...
        return this.dataRepository.findAll(); // -> return 타입 List<T>
    ...
    //  * @return
    //  */
    // public String setMemberUpdateGet(String data_id, String mem_name) {
    //     log.info("mem_id=[%s], mem_name=[%s]".formatted(data_id, mem_name));
    ...
    //         Data data_update = data.get();
    //         data_update.setData_name(mem_name);
    //         this.dataRepository.save(data_update);

    //         return "회원 아이디 [" + data_id + "]에 대한 정보가 수정되었습니다.";
    //     }

    //     return "회원 아이디 [%s]에 대한 정보가 존재하지 않습니다.".formatted(mem_id);
    // }

    // public Data setMemberUpdate(Data p_member) {
    //     log.info("mem_id=[%s], mem_name=[%s]".formatted(p_member.getMem_id(), p_member.getMem_name()));

    //     Optional<Data> member = this.dataRepository.findById(p_member.getMem_id());
    ...
    //         return this.dataRepository.save(member_update);
    //     }
